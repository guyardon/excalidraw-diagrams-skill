...
| Inline `max-height` override has no effect | Width is the binding constraint, not height. The diagram fills 100% width and height follows from aspect ratio. **Fix the canvas dimensions** — use a narrower canvas (600-700px) so the aspect ratio produces a taller render. CSS `max-height` only helps when height is the binding constraint. | 1, 23 |
